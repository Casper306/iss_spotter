...
request('https://api.ipify.org?format=json', (error, response, body) => {
  ...
  if (response.statusCode !== 200) {
    callback(Error(`Status Code ${response.statusCode} when fetching IP: ${body}`), null); // Print the response status code if a response was received
    return;
  }
 ...
 */
 const fetchCoordsByIP = function(ip, callback) {
  ...
  //console.log(typeof ip.toString() );
